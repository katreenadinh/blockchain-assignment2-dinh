...
const { abi } = await artifacts.readArtifact("CampusCredit");
    ...
    const acct2 = (ACCT2 || account.address) as `0x${string}`;
    const balances = async (label: string) => {
        const b1 = await publicClient.readContract({
            address: getAddress(TOKEN), abi,
            functionName: "balanceOf", args: [account.address]
        });
        const b2 = await publicClient.readContract({
            address: getAddress(TOKEN), abi,
            functionName: "balanceOf", args: [acct2]
        });
        console.log(`${label} | Deployer: ${formatUnits(b1 as bigint, DECIMALS)} CAMP | Acct2:
${formatUnits(b2 as bigint, DECIMALS)} CAMP`);
    };
